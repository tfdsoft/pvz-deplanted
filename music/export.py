...
asmMusicDataName = fr'music_data_pvz_deplanted_{exportStemPrefix}'
...
if __name__ == "__main__":
    ...
    if extMeta:
        ...
        maxDataInBank = 8192 - (instsize / 1.5 * 3)
    ...
    for i in range(1, len(bins)):   # The first music bank is special
        header_music_bank_data += [
            f'.segment "{datBankSegPrefix}{i+firstMusBank}"',
    ...
    if (len(dpcmBanks)):
        header_dmc_bank_data = [
            '', '; DMC banks',
            f'.segment "{datBankSegPrefix}{firstDmcBank}"',
            '\tfirstDMCBankPtr := *',
        ...
        for i, bank in enumerate(dpcmBanks[1:], 1):
            if bank != dmcBankMetaUnused:
                header_dmc_bank_data += [
                    f'.segment "{datBankSegPrefix}{i+firstDmcBank}"',
    ...
    header_data = [
        ...
        '', '; Music data banks',
        f'.segment "{datBankSegPrefix}{firstMusBank}"',
        '\tfirstMusicBankPtr := *',
